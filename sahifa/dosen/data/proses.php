...
if (isset($_GET['ambil'])) {
    $menu = "";
    $ambil = $db->query("SELECT app_dosen.nip, app_dosen.nama_lengkap, app_dosen.email, app_dosen.hp, akademik_prodi.nama_prodi
                         from app_dosen
                         INNER JOIN akademik_prodi ON app_dosen.prodi_id = akademik_prodi.prodi_id");
    while ($row = $ambil->fetch(PDO::FETCH_BOTH)) {
        ...
        $data[] = array($row['nip'], $row['nama_lengkap'], $row['email'], $row['hp'], $row['nama_prodi'], $menu);
    }
